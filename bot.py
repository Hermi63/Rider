...
FIREBASE_KEY_JSON = os.environ.get("FIREBASE_KEY_JSON", "")  # JSON строка
FIREBASE_KEY_B64  = os.environ.get("FIREBASE_KEY_B64",  "")  # base64 (Railway)
...
# Firebase — читаем ключ из base64, JSON или файла
import json, base64
if FIREBASE_KEY_B64:
    key_json = base64.b64decode(FIREBASE_KEY_B64).decode("utf-8")
    cred = credentials.Certificate(json.loads(key_json))
    logger.info("Firebase: ключ из base64")
elif FIREBASE_KEY_JSON:
    cred = credentials.Certificate(json.loads(FIREBASE_KEY_JSON))
    logger.info("Firebase: ключ из JSON")
else:
